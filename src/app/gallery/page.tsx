  ...
  useEffect(() => {
    const loadImages = async () => {
      try {
        ...
        // Only process original images, not the _400x400 thumbnails generated by the extension
        const originals = result.items.filter(
          (item) => !/_\d+x\d+\.[^.]+$/.test(item.name)
        );
        ...
        const photoData: PhotoData[] = await Promise.all(
          originals.map(async (itemRef) => {
            ...
            // Construct thumbnail path: e.g. DSC00064.jpg → DSC00064_400x400.jpg
            const dotIndex = itemRef.name.lastIndexOf(".");
            const thumbName =
              itemRef.name.slice(0, dotIndex) + "_400x400" + itemRef.name.slice(dotIndex);
            const thumbUrl = await getDownloadURL(ref(storage, `gallery/${thumbName}`)).catch(
              () => fullUrl // fall back to full-size if thumbnail doesn't exist yet
            );
